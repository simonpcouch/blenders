...
wf_set_configs <-
  tibble(
    workflow_set = rep(wf_sets, times = nrow(test_specs) + 1),
    data = rep(test_data$data, times = nrow(test_specs) + 1),
    meta_learner = c(
      rep(test_specs$spec, each = nrow(test_data)), 
      rep(list(NULL), each = nrow(test_data))
    ),
    steps = list(tibble())
  )
...
res_stack_small <-
  res_stack %>%
  mutate(
    outcome = rep(test_data$outcome, times = nrow(test_specs) + 1)
  ) %>%
  rowwise() %>%
  mutate(
...
save(res, file = "data-raw/res.Rda")
```
