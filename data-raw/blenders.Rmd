...
wf_set_configs <-
  tibble(
    workflow_set = rep(wf_sets, times = nrow(test_specs)),
    data = rep(test_data$data, times = nrow(test_specs)),
    meta_learner = c(
      rep(test_specs$spec, each = nrow(test_data))
    ),
    steps = list(tibble())
  )
...
res_stack <-
  res_st %>%
  bind_rows() %>%
  bind_cols(wf_set_configs) %>%
  select(-c(workflow_set, data))

save(res_stack, file = "data-raw/res_stack.Rda")
```


```{r}
# putting together a more plottable data frame
res_base_small <-
  res_base %>%
  rowwise() %>%
  mutate(
    model_type = class(spec)[1],
    dataset = case_when(
      outcome == "latency" ~ "Tree Frogs",
      outcome == "compressive_strength" ~ "Concrete",
      outcome == "Sale_Price" ~ "Ames Housing",
      outcome == "outcome" ~ "Simulated",
    ),
    stack = FALSE
  ) %>%
  select(-steps, -spec, -outcome)
...
res_stack_small <-
  res_stack %>%
  mutate(
    outcome = rep(test_data$outcome, times = nrow(test_specs))
  ) %>%
  rowwise() %>%
  mutate(
    model_type = class(meta_learner)[1],
    dataset = case_when(
      outcome == "latency" ~ "Tree Frogs",
      outcome == "compressive_strength" ~ "Concrete",
      outcome == "Sale_Price" ~ "Ames Housing",
  ...
  select(-steps, -meta_learner, -outcome)

res_stack_small

stack_benchmarks <-
  bind_rows(res_base_small, res_stack_small) %>%
  select(stack, model_type, dataset, metric, metric_value, time_to_fit) %>%
  ungroup()

save(stack_benchmarks, file = "data-raw/stack_benchmarks.Rda")

usethis::use_data(stack_benchmarks, overwrite = TRUE)
```
